...
export default function SystemPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [{ data: { user } }, h, p] = await Promise.all([supabase.auth.getUser(), fetch("/api/health", { cache: "no-store" }), fetch("/api/push/vapid-public", { cache: "no-store" })]);
      ...
      const missing = featureChecks.filter(([, error]) => Boolean(error)).map(([name]) => name);
  ...
  const rows = useMemo(() => [
    ...
    { label: "Feature schema", status: schemaReady === null ? "loading" as Status : schemaReady ? "ok" as Status : "error" as Status, detail: schemaReady ? "Reminder, notification event, dan push table tersedia." : `Migration belum lengkap: ${schemaMissing.join(", ")}.`, icon: Sparkles },
    ...
    { label: "Reminder Engine", status: push?.cronConfigured ? "ok" as Status : "warn" as Status, detail: push?.cronConfigured ? "Cron secret tersedia." : "Cron server belum dikonfigurasi; mode saat tab terbuka tetap dapat memproses reminder.", icon: TimerReset },
  ...
  ], [health, loading, push, counts.subscriptions, swReady, deviceOnline, schemaReady, schemaMissing]);
  ...
  const setupText = `VAPID_SUBJECT=mailto:admin@domain-kamu\nVAPID_PUBLIC_KEY=...\nVAPID_PRIVATE_KEY=...\nSUPABASE_SERVICE_ROLE_KEY=...\nLICIA_CRON_SECRET=...`;
  ...
  return <div className="system-v29 space-y-5 animate-licia-page-in">
